...
<body>
	...
	<div id="wrapper">
		<div id="logo"></div>
		<div id="title"><img src="/img/openStat.svg"><span style="font-size: 0.8rem">openStat-Instanz</span><br />openPSZ</div>	
		<div id="title_after"></div>
		<div id="actionneeded">
			<?php 
				if ( isset($_SESSION['e']) AND $_GET['e'] == "noextension" ) { ?>
					<p>Bitte aktiviere die <b>openPSZ</b>-Erweiterung für Firefox.</p>
				...
				if ( isset($_SESSION['e']) AND $_GET['e'] == "extensionupdate" ) { ?>
					<p>Bitte aktualisiere die <b>openPSZ</b>-Erweiterung für Firefox:</p>
					<p><a href="https://addons.mozilla.org/addon/openstat/">https://addons.mozilla.org/addon/openstat/</a></p>
					<p>und <b>erlaube die Aktivierung der Erweiterung in privaten Fenstern</b>.</p>
				<?php };
				if ( isset($_SESSION['e']) AND $_GET['e'] == "notprivate" ) { ?>
					Bitte öffne <b>openPSZ</b> in einem privaten Fenster (Strg+Umschalt+P).
				<?php };
			 ?>
		</div>
		<div id="error">
...
<!--			<input id="test" type="checkbox" hidden onclick="this.closest('form').submit();" <?php echo($disabled); ?>><br /><br /> -->
	...
	<script>
        ...
        if ( <?php echo($firefox_least_featureversion); ?> > parseInt(window.navigator.userAgent.replace(/.*Firefox\//,'').replace(/\..*/,'')) ) {
            document.querySelector('#actionneeded').innerHTML += '<p>Diese Firefoxversion ist zu alt. Es kann sein, dass manche Funktionen nicht verfügbar sind oder fehlerhaft reagieren. Bitte aktualisiere Firefox.</p>';
        }
    </script>
</body>
