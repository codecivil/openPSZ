...
INSERT  INTO `opsz_aufnahme_permissions` (`keymachine`,`keyreadable`,`realid`,`typelist`,`edittype`,`referencetag`) SELECT 'suizidversuche','Suizidversuche: Wer + Wann + Wo + Wie','37.2','TEXT','EXTENSIBLE LIST + DATE + TEXT + TEXT; MULTIPLE','_suizidwer' FROM DUAL WHERE (SELECT count(*) FROM information_schema.COLUMNS WHERE TABLE_NAME = 'opsz_aufnahme' AND COLUMN_NAME = 'suizidversuche') > 0 AND (SELECT count(keymachine) FROM `opsz_aufnahme_permissions` where keymachine = 'suizidversuche') = 0;
...
INSERT  INTO `opsz_aufnahme_permissions` (`keymachine`,`keyreadable`,`realid`,`typelist`,`edittype`) SELECT 'suizidrisikofaktoren','Sonstige besondere Risikofaktoren','37.3','TEXT','FREE' FROM DUAL WHERE (SELECT count(*) FROM information_schema.COLUMNS WHERE TABLE_NAME = 'opsz_aufnahme' AND COLUMN_NAME = 'suizidrisikofaktoren') > 0 AND (SELECT count(keymachine) FROM `opsz_aufnahme_permissions` where keymachine = 'suizidrisikofaktoren') = 0;
ALTER TABLE `opsz_aufnahme` ADD COLUMN `suizidale_absichten` TEXT DEFAULT NULL;
INSERT  INTO `opsz_aufnahme_permissions` (`keymachine`,`keyreadable`,`realid`,`typelist`,`edittype`,`referencetag`) SELECT 'suizidale_absichten','suizidale Absichten: Absicht oder Handlungspläne + Glaubhaft distanziert + Datum + imperative Stimmen + Absprachefähig + Folgende Absprachen wurden getroffen + Aufnahme in Klinik besprochen + Aufnahme in Klinik veranlasst','37.100','TEXT','LIST + LIST + DATE + LIST + LIST + TEXT + TEXT + LIST; MULTIPLE','_suizidalitaet + _suizid_binary + _ + _yesno + _suizid_binary + _ + _ + _yesno' FROM DUAL WHERE (SELECT count(*) FROM information_schema.COLUMNS WHERE TABLE_NAME = 'opsz_aufnahme' AND COLUMN_NAME = 'suizidale_absichten') > 0 AND (SELECT count(keymachine) FROM `opsz_aufnahme_permissions` where keymachine = 'suizidale_absichten') = 0;
...
INSERT  INTO `opsz_aufnahme_permissions` (`keymachine`,`keyreadable`,`realid`,`typelist`,`edittype`,`referencetag`) SELECT 'inaktivseit','inaktiv seit: Quartal + Jahr','2.700','TEXT','LIST + LIST; MULTIPLE','_quartal + _jahr' FROM DUAL WHERE (SELECT count(*) FROM information_schema.COLUMNS WHERE TABLE_NAME = 'opsz_aufnahme' AND COLUMN_NAME = 'inaktivseit') > 0 AND (SELECT count(keymachine) FROM `opsz_aufnahme_permissions` where keymachine = 'inaktivseit') = 0;
